...
import { getAssetBuffer } from "./clara-asset-manager.js";
...
function createContextInfo(jid, text, title = "", body = "", thumbnail = null) {
  ...
  if (jid && text) {
    ...
    contextInfo.stanzaId = "CLARAAI" + Date.now();
  }
...
async function serialize(sock, msg, store = {}) {
  ...
  m.botName = sock.user?.name || config.bot?.name || "Clara-AI";
  m.messageId = m.id;
  m.chatId = m.chat;
  m.senderId = m.sender;
    ...
    if (replyVariant === 2) {
      ...
      return sock.sendMessage(
        m.chat,
        {
          ...
          jpegThumbnail: srtImage ? await sharp(srtImage).resize(300, 300).toBuffer() : await sharp(getAssetBuffer("clara2"))
            .resize(300, 300)
            .toBuffer(),
          caption: text,
          ...defaultOptions,
      ...
      const sss = getAssetBuffer("clara3");
      return sock.sendMessage(
        m.chat,
        {
          video: getAssetBuffer("clara-mp4"),
          caption: text,
          gifPlayback: true,
          contextInfo: {
            ...contextInfo,
      ...
      const thumbnail = srtImage || getAssetBuffer("clara");
        ...
        },
        {
          ...
          contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
              ...
              newsletterName: config.saluran?.name || config.bot?.name || "Clara-AI",
      ...
      const thumbnailBuf = srtImage || getAssetBuffer("clara");
      const fakeOrder = {
        key: {
          participant: "0@s.whatsapp.net",
          remoteJid: m.chat,
        ...
        message: {
          orderMessage: {
            ...
            status: 1,
            surface: 1,
            message: config.bot?.name || "Clara-AI",
