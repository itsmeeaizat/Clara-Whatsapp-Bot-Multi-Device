import {
  ...
  getRandomReward,
  getProgressiveHint,
} from "./clara-game-data.js";
import { getDatabase } from "./clara-database.js";
import { addExpWithLevelCheck } from "./clara-level.js";
import {
  getGameContextInfo,
  sendGamePreview,
  checkFastAnswer,
} from "./clara-context.js";
import config from "../../config.js";
import fs from "fs";
import sharp from "sharp";
let fetchBuffer;
try {
  fetchBuffer = (await import("./clara-utils.js")).fetchBuffer;
} catch { }
        ...
        if (session && session.gameType === gameType) {
          ...
          await sock.sendPreview(
            m.chat,
            {
              ...
              jpegThumbnail: await sharp(fs.readFileSync(config.assets["clara2"])).resize(300, 300).toBuffer(),
              previewType: 0,
            },
          );
          return;
...
const games = new ClaraGames();

export { ClaraGames, games };
