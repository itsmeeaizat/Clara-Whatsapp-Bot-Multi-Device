...
function tempPath(ext) {
  ensureTmp();
  return path.join(TMP_DIR, `upimg_${Date.now()}_${Math.random().toString(16).slice(2)}${ext}`);
}
...
async function handler(m, { sock, config: botConfig }) {
  try {
    ...
    if (!media) {
      const text =
        alyaHeader("Upload Image", "🖼️") +
        "\n\n" +
        bracketBox("🖼️", "ɪɴꜱᴛʀᴜᴋsɪ", [
          "◦ Kirim gambar + caption .upimg",
          "◦ Atau reply gambar dengan .upimg",
          "◦ Format: PNG, JPG, WEBP",
        ]) +
        "\n\n" +
        separator() +
