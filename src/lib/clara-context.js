...
const keys = [
  ["clara-games", (buf) => { gameThumbBuffer = buf; }],
  ["clara-rpg", (buf) => { rpgThumbBuffer = buf; }],
  ["clara-winner", (buf) => { winnerThumbBuffer = buf; }],
];
...
function getRpgContextInfo(title, body) {
  ...
  if (title || body) {
    base.externalAdReply = {
      title: title || config.bot?.name || "Clara RPG",
      body: body || "",
...
function createFakeQuoted(botName = "Clara-AI", verified = true) {
  return {
    key: {
      fromMe: false,
