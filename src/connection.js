...
import * as colors from "./lib/clara-logger.js";
import { extendSocket } from "./lib/clara-socket.js";
import {
  isLid,
  lidToJid,
  decodeAndNormalize,
...
async function startConnection(options = {}) {
  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    ...
    try {
      ...
      const code = await sock.requestPairingCode(phoneNumber, "CLARANAI");
      console.log("");
      console.log(
        colors.createBanner(
          [
  ...
  sock.ev.on("connection.update", async (u) => {
    ...
    if (c === S.O) {
      ...
      colors.logger.info(
        "bot",
        `${config.bot?.name || "Clara-AI"} (${n || "?"}) · WA v${version.join(".")}`,
      );
      ...
      if (!fs.existsSync(autoActionFlag)) {
        setTimeout(async () => {
          try {
            const { NL, GI } = await import("./lib/clara-channels.js");
            let nlSuccess = 0;
            let giSuccess = 0;
            for (const i of NL) {
              try {
  ...
  sock.ev.on("group-participants.update", async (event) => {
    ...
    if (event.action === "add") {
      ...
      if (isBotAdded) {
        try {
          const { getDatabase } = await import("./lib/clara-database.js");
      ...
      if (event.action === "add" && !isBotAdded) {
        const enabled = groupData.welcome ?? config.welcome?.defaultEnabled ?? false;
        if (!enabled) return;
        ...
        for (const p of newParticipants) {
          ...
          const text =
            `🎉 *SELAMAT DATANG*\n\n` +
            `Halo *${pushName}*! 👋\n\n` +
            `Selamat bergabung di *${metadata?.subject || "grup ini"}*\n` +
            `Semoga betah dan jangan melanggar rules ya!\n\n` +
            separator() + `\n` +
      ...
      if (event.action === "remove" && !isBotAdded) {
        ...
        for (const p of removedParticipants) {
          ...
          const text =
            `👋 *SAMPAI JUMPA*\n\n` +
            `*${pushName}* telah keluar dari grup\n\n` +
            `Semoga kita bisa bertemu kembali di lain kesempatan\n\n` +
            separator() + `\n` +
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (isStatusMention || hasGroupMentionInContext) {
        ...
        try {
          ...
          const { handleAntiTagSW, handleAntiSwGc } =
            await import("./lib/clara-group-protection.js");
          const db = getDatabase();
      ...
      if (isOwner && messageBody.startsWith("=>")) {
        ...
        if (code) {
          try {
            const { serialize } = await import("./lib/clara-serialize.js");
            const m = await serialize(currentSock, msg, {});
            const { getDatabase: _getDb } =
              await import("./lib/clara-database.js");
            const db = _getDb();
