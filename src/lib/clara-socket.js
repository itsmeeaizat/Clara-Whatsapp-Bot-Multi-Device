...
import { addExifToWebp, DEFAULT_METADATA } from "./clara-exif.js";
...
async function extendSocket(sock) {
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "Clara-AI";
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = config.bot?.name || "Clara-AI";
    if (source) {
      let data = source;
