...
const pluginConfig = {
  name: "fightgriffin",
  alias: ["fightgriffin", "fightgrif"],
  category: "rpg",
  description: "Pertarungan pet Griffin melawan Griffin liar",
  usage: ".fightgriffin",
  example: ".fightgriffin",
  isOwner: false,
  isPremium: false,
  isGroup: false,
  isPrivate: false,
...
async function handler(m, { sock }) {
  try {
    ...
    if (isWin) {
      ...
      await m.reply(
        `🦅 *PERTARUNGAN GRIFFIN BERHASIL!*\n\n` +
        `⚔️ Griffin kamu (Lvl ${griffinLevel}) menang melawan Griffin liar (Lvl ${enemyLevel})!\n\n` +
        `🎁 *Hadiah:*\n` +
      ...
      const penaltyMoney = Math.min(user.money || 0, Math.floor(Math.random() * 10000) + 5000);
      user.money = (user.money || 0) - penaltyMoney;
      await db.write();

      await m.reply(
        `🦅 *PERTARUNGAN GRIFFIN KALAH!*\n\n` +
        `💥 Griffin kamu (Lvl ${griffinLevel}) kalah dari Griffin liar (Lvl ${enemyLevel})!\n\n` +
